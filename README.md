...
| md/PDF 문서 길이를 수용하는 Context Length | `ollama show` | ✅ 131,072 토큰 | ✅ 32,768 토큰 |
...
선정한 Use Case(문서 요약·분석·QA)에 맞는 고정 질문 세트 10개를 만들고, 모든 로컬 후보(EXAONE3.5, granite4.1)에 동일하게 적용합니다. 추가 질문은 선택이며 기본 10개의 집계와 구분합니다.
...
> **비정상 사례를 추가한 이유**: 사전 테스트에서 "URL을 참고해서 답해달라"는 질문에 두 모델 모두 실제로 URL에 접근하지 않았음에도 마치 확인한 것처럼 답변하는 현상을 관찰했습니다. 로컬 LLM은 인터넷 접근 권한이 없는데도(본 프로젝트 실행 환경 기준 확인됨), 일부 모델은 이를 인정하지 않고 그럴듯한 답을 지어내는 경향을 보여, 이를 정식 평가 항목으로 포함합니다.

### 소스 문서

| 문서 ID | 파일명 | 내용 |
|---|---|---|
| DOC-A | (readfiles 폴더 내 지정) | granite/IBM 관련 Model Card 원문 |
| DOC-B | (readfiles 폴더 내 지정) | EXAONE Model Card 원문 |
...
이번 Use Case(수업자료/README 문서 분석)는 EXAONE3.5(32K)·granite4.1(131K) 두 모델 모두 처리 가능한 범위 내 문서 길이로 제한합니다. 별도의 초장문(수만 토큰 이상) 실험은 이번 필수 질문 세트에 포함하지 않습니다.

### 품질 채점 기준 (STEP2 선호순위 반영)
...
| Q3 | gpt-5.6-luna | 4 | 4 | 1 | **0** | 125B, 6B를 정확히 지정하였지만, CHATGPT특징인 불필요한 추가답변이 거슬렸음. |
...
| Q4 | gpt-5.6-luna | 4 | 2 | 2 | **10** | 기본토큰개수와 확장토큰개수와 추가설명까지 원하는 대로 결과가 나옴 |
...
| Q9 | gpt-5.6-luna | 4 | 4 | 2 | **8** | DOC-A, DOC-B 문서구분을 정확히하였고, 문서의 특징과 전혀 연관없다고 생각한 두 문서의 공통점을 찾았음.  |
| Q10 | exaone3.5:7.8b | 4 | 3 | 2 | **9** | 요청하지 않은 markdown 코드블럭이 들어간거 제외하면 전체적으로 만족. |
| Q10 | granite4.1:3b | 4 | 2 | 2 | **8** | 전체적으로 만족스러운 답변 |
| Q10 | gpt-5.6-luna | 4 | 4 | 2 | **10** | 대답품질에 만족|

결과 : 
1. Q1 : Cloud Model도 Tool에 인터넷 허용권한을 주지 않아 해당url에 접근이 불가능하였는데도, 접근해서 대답한것처럼 응답하였음.
2. Q7 : 3~4문장이라고 정의하니 콤마까지 사용하여 3~4문장은 맞지만 글씨개수가 상당히 많게 응답이옴. 추후 짧게 요약할경우 문장수 + 글자개수를 정해야겠다는 결과가 나옴
3. Result : Cloud Model의 경우 간단한 프롬프팅으로도 평균이상의 답변을 받았음. 여유가 된다면 전반적으로 할루시네이션을 최대한 없앤 프롬프트 튜닝버전을 테스트해볼예정
