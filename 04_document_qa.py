...
QUESTIONS = [
    ...
    },
    {
        ...
        "source_doc_list": ["DOC-A"],
        "question": (
            "문제3 : DOC-A에서 이 모델의 전체 파라미터 수와 한 번의 추론에서 "
            "실제로 활성화되는 파라미터 수를 각각 찾아 주세요. "
    ...
    },
    {
        ...
        "source_doc_list": ["DOC-A"],
        "question": (
            "문제4 : DOC-A에서 이 모델의 기본 Context Length와 확장 가능한 "
            "최대 Context Length를 각각 찾아 주세요. "
            "'기본 Context Length: 값 / 확장 최대 Context Length: 값' "
            "형식으로 한 줄만 답하고, 서로 다른 수치를 혼동하지 마세요."
        ),
    },
    {
        "id": "Q5",
    ...
    },
    {
        ...
        "source_doc_list": ["DOC-A"],
        "question": (
            "문제6 : DOC-A에 이 모델의 정확한 출시일이 연/월/일 형식으로 "
            "명시되어 있는지 확인해 주세요. 정확한 날짜가 없다면 추측하거나 "
            "외부 지식을 사용하지 말고 "
            "'제공된 문서에 정확한 출시일(연/월/일)은 명시되어 있지 않습니다.'라고 "
            "한 문장으로 답해 주세요."
        ),
    },
    {
        ...
        "question": (
            "문제7 : DOC-B에 명시된 내용만 사용하여 초보자가 이해하기 쉽게 "
            "정확히 3~4문장으로 요약해 주세요. "
            "API 재시도, 백오프, fallback의 역할을 포함하되 "
            "문서에 없는 동작이나 설정은 추가하지 마세요."
        ),
    },
    {
        "id": "Q8",
        ...
        "source_doc_list": ["DOC-B"],
        "question": (
            "문제8 : DOC-B에서 OpenAI Python SDK가 기본적으로 재시도하는 "
            "오류의 종류와 기본 재시도 횟수를 찾아 주세요. "
    ...
    },
    {
        ...
        "source_doc_list": ["DOC-B"],
        "question": (
            "문제10 : DOC-B에 명시된 재시도가 불가능하여 즉시 중단해야 하는 "
            "대표적인 오류를 정확히 3개 선택해 마크다운 표로 작성해 주세요. "
            "표의 열은 '오류'와 '즉시 중단해야 하는 이유' 두 개만 사용하세요. "
            "표 앞뒤에 설명을 추가하지 말고, 마크다운 코드 블록으로 감싸지 마세요."
        ),
    },
]
        ...
        for q in QUESTIONS:
            ...
            for model in MODELS:
                    ...
                    if missing:
                        ...
                        try:
                            response = client.chat(model=model, messages=[{"role": "system", "content": SYSTEM_PROMPT},{"role": "user", "content": prompt}],
