...
MorphFactory Motif 3에 묻고 결과를 JSONL로 저장하는 스크립트.

사용법:
1. 이 파일과 같은 위치에 readfiles/ 폴더를 만들고 DOC-A*, DOC-B* 파일을 넣는다.
2. 실행 시 Morph API 키를 화면에 표시되지 않는 방식으로 입력한다.
3. 질문 10개를 Motif 3에 각각 한 번씩 보내고 결과를 저장한다.
"""
...
RESULT_FILE = PROJECT_DIR / "log_06_motif_document_qa.jsonl"

# MorphFactory의 공개 퀵스타트에서 확인된 Chat Completions 형식만 사용합니다.
# 출력 토큰 제한을 추가하려면 API 지원 여부를 확인한 뒤
# MODEL_OPTIONS에 "max_tokens": 1024를 추가하세요.
MODEL_OPTIONS = {}

MODEL_COMPARISON_INFO = {
    "model": MODEL,
    "service_type": "Cloud API",
    "license": "공개 정보에서 확인 필요",
    "전체_파라미터": "314B MoE",
    "활성_파라미터": "13.2B",
    "모델 최대 Context Length": None,
    "최대 출력 토큰": None,
    "model_card_source": "https://cbt.morphfactory.io/",
}
...
def main() -> None:
    print("06_motif_document_qa.py 파일 실행")
    ...
    try:
        ...
        for question in QUESTIONS:
            ...
            if missing:
                ...
                try:
                    ...
                    record["status"] = "성공"
                    record["response"] = choice.message.content or ""
                    record["API_처리상태"] = choice.finish_reason

                    # ---- 디버그: 숨겨진 필드(reasoning 등) 확인용 ----
                    message_dict = choice.message.model_dump(mode="json")
                    # content 외에 다른 키가 있는지 전부 기록
                    hidden_keys = {
                        k: v for k, v in message_dict.items()
                        if k != "content" and v not in (None, "", [], {})
                    }
                    if hidden_keys:
                        record["숨겨진_필드_감지"] = hidden_keys
                    # 원본 응답 전체를 별도 파일에 통째로 남김 (필요할 때만 열어보면 됨)
                    with (PROJECT_DIR / "debug_raw_responses.jsonl").open(
                        "a", encoding="utf-8"
                    ) as debug_file:
                        debug_file.write(
                            json.dumps(
                ...
                #     record["API_처리상태"] = choice.finish_reason
                    ...
                    if usage is not None:
                        record["입력_토큰수"] = getattr(
                            usage,
                            "prompt_tokens",
                            None,
                        )
                        record["출력_토큰수"] = getattr(
                            usage,
                            "completion_tokens",
                            None,
                        )
                        record["전체_토큰수"] = getattr(
                            usage,
                            "total_tokens",
                            None,
                        )
                        ...
                        if hasattr(usage, "model_dump"):
                            record["토큰_사용량_상세"] = usage.model_dump(
                                mode="json"
                            )
                    else:
                        record["입력_토큰수"] = None
        ...
        append_run(run)
        print("06_motif_document_qa.py 파일 종료")
